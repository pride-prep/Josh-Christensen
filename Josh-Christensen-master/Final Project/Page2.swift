...
class Page2: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for meal in Meals {
            ...
            } else if !meal.peanuts && PeanutFilter {
                fillteredMeals.append(meal)
            } else if meal.peanuts && !PeanutFilter {
                fillteredMeals.append(meal)
            }
            
            if meal.treenuts && TreenutFilter {
                
            } else if !meal.treenuts && TreenutFilter {
                fillteredMeals.append(meal)
            } else if meal.treenuts && !TreenutFilter {
                fillteredMeals.append(meal)
            }
            
            if meal.vegetarian && VegetarianFilter {
                
            } else if !meal.vegetarian && VegetarianFilter {
                fillteredMeals.append(meal)
            } else if meal.vegetarian && !VegetarianFilter {
                fillteredMeals.append(meal)
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let page3 = segue.destination as? Page3 {
            // list all variables from your second view controller
            page3.text = MealName.text
